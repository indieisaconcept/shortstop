...
var regex = (function () {
    ...
    is = function (name, evidence, strict) {
        var type, result;

        name = name.toLowerCase();
        type = Object.prototype.toString.call(evidence).match(/^\[object\s(.*)\]$/)[1].toLowerCase();

        return strict != null && !strict ? typeof evidence === name : type === name;
    };
...
exports.create = function (parent) {
    ...
    return {
        ...
        use: function (protocol, filter, impl) {
            ...
            impl = is('function', filter) ? filter : impl;

            protocols = is('array', filter) ? [protocol].concat(filter) : [protocol];
            handlers = this._handlers;
            handler = handlers[protocol];

            if (!handler) {
                rx = is('string', filter) ? new RegExp(filter.replace('{protocol}', protocol)) : regex('protocol', protocols.join('|'));
        ...
        resolve: function resolve(src) {
            ...
            if (is('object', src, false) && src !== null) {
